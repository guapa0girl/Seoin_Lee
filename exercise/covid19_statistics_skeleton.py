def normalize_data(n_cases, n_people, scale):
    ...
    for idx, n in enumerate(n_cases):
        normalized = (n/n_people[idx]) * scale 
        norm_cases.append(normalized)
    return norm_cases
...
print('| ------ | --------- | --------- | -------------- |')
for idx, new in enumerate(n_covid):
    covid_ratio = (new /sum_covid) * 100 
    print('| %s | %d | %.1f | %.1f' % (regions[idx], new, covid_ratio, norm_covid[idx])) 
print()